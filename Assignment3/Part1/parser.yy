...
Goal:
  MainClass END  {$$ = new Node("Program", ""); $$->children.push_back($1); root = $$;}
  | MainClass ClassDeclarationP END {$$ = $2; $$->children.push_front($1); root = $$;};

MainClass: 
  CLASS Identifier LBRACE PUBLIC STATIC VOID MAIN LPAR STRING LBRACKET RBRACKET Identifier RPAR LBRACE Statement RBRACE RBRACE {$$ = new Node("Main Class", $2->value); $$->children.push_back($12); $$->children.push_back($15);};

ClassDeclarationP:
  ClassDeclaration {$$ = new Node("Program", ""); $$->children.push_back($1);}
  | ClassDeclarationP ClassDeclaration {$$ = $1; $$->children.push_back($2);};

ClassDeclaration: CLASS Identifier ClassBody {$$ = new Node("Class", $2->value); $$->children.push_back($3);}
| CLASS Identifier EXTENDS Identifier ClassBody {$$ = new Node("Class", $2->value); $$->children.push_back($4); $$->children.push_back($5);};
...
MethodDeclaration:
PUBLIC Type Identifier LPAR RPAR MethodBody {$$ = new Node("Method", $3->value); $$->children.push_back($2); $$->children.push_back($6);}
| PUBLIC Type Identifier LPAR MethodDeclarationParams RPAR MethodBody {$$ = new Node("Method", $3->value); $$->children.push_back($2); $$->children.push_back($5); $$->children.push_back($7);};
...
MethodParam:
  Type Identifier {$$ = new Node("Variable", ""); $$->children.push_back($1); $$->children.push_back($2);};
...
Type: INT LBRACKET RBRACKET {$$ = new Node("Type", "int[]");}
  ...
  | Identifier {$$ = new Node("Type", $1->value);};
...
LRStatement: 
  Statement {$$ = new Node("Statements", ""); $$->children.push_back($1);}
| LRStatement Statement { $$ = $1; $$->children.push_back($2);};
...
comExpression: Expression {$$ = new Node("Params", ""); $$->children.push_back($1);}
  | comExpression COMMA Expression {$$ = $1; $$->children.push_back($3);};
